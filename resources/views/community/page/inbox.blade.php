...
<div class="modal-body">
    <div class="row">
        @if(isset($totalMessages) && $totalMessages > $nbDisplayed)
            <p class="text-center">
                <a href="{{ url()->route('community.inbox', ['idCommunity' => $community->id, 'full' => 1 ] ) }}" class="btn btn-border-default float-right fn-reload-modal">
        ...
        @endif
            ...
            @foreach($feed AS $message)
                ...
                @if($type == 6 && $message->sender_id == auth()->auth()->guard('web')->user()->id && $message->sender_type == 'User' )
...
(function($){
    $(document).on('click', '.fn-reload-modal', function(e){
        ...
        $.get('{{ url()->to('/') }}' + laroute.route('community.inbox', params))
