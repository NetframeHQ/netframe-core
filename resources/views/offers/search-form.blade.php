{{ Form::open(array('id' => 'offersForm', 'name' => 'offersForm')) }}
<div class="well well-sm clearfix">
    <div class="row visible-xs">
        <div class="form-group col-md-12">
            <a href="{{ url()->route('offer_edit', ['user', auth()->guard('web')->user()->id]) }}" class="btn btn-border-default float-right" data-toggle="modal" data-target="#modal-ajax">
                <span class="icon ticon-publish"></span>
                {{ trans('netframe.publishOffer') }}
            </a>
        </div>
    </div>
            ...
            <div class="btn-group" data-toggle="buttons">
                @foreach($offersTypeChoice as $typeO=>$parametersO)
                    <label class="btn btn-default">
                        {{ Form::radio('offer_choice', $typeO) }}
                        {{ trans('offer.choiceSearch.'.$typeO) }}
                    </label>
                @endforeach
            </div>
        </div>
