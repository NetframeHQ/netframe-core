...
(function($) {
    ...
    distanceElement = $('#eventsForm').find('input[name="distance"]');
  ...
  //implement google searchplaces
    ...
    $(document).on('submit', '#eventsForm', function(event) {
        ...
        var actionUrl = '{{ url()->to('/') }}' + laroute.route('search_events');
        var _form = $(this);
    ...
    var stopScroll = 0;
    var formSearch = $('#eventsForm');

    window.onscroll = function(ev){
        ev.stopPropagation();
        if((window.innerHeight + window.pageYOffset + 1) >= document.body.offsetHeight) {
            var lastPostDate = $('#events-results article').last().data('time');
            ...
            $.post('{{ url()->to('/') }}' + laroute.route('search_events'), formData)
                .success(function (data) {
                    $("#events-results").append(data.view);
                    new PlayMediaModal({
                        $modal: $modal,
